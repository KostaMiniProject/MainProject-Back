...
//        List<ExchangePost> initiatedExchanges = user.getExchangePosts().stream().filter(ex -> ex.getExchangePostStatus().equals(ExchangePost.ExchangePostStatus.COMPLETED)).toList();
//        List<Bid> participatedExchanges = user.getBids().stream().filter(b -> b.getStatus().equals(Bid.BidStatus.COMPLETED)).toList();
